...
{
	initialiseGLFW();
	GLFWwindow * window = createWindow("8. Camera");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (unsigned int i = 0; i < 10; i++)
        {
            ...
            //float angle = 20.0f * i * (float)(sinf(glfwGetTime()) / 2.0f + 0.5f);
            float angle = 20.0f * i;
...
GLFWwindow * createWindow(const std::string & name)
{
	...
	GLFWwindow * window = glfwCreateWindow(WIDTH, HEIGHT, "8. Camera", NULL, NULL);
	if (window == NULL)
	{
    ...
    // resize the frame based on framebuffer_size_callback
